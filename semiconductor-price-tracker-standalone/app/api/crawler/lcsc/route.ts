...
export async function POST(request: Request) {
  ...
  if (isDevelopment) {
    console.log(`[LCSC API] POST ${JSON.stringify({ ids: body.ids })}`);
  }
  ...
  const results = await Promise.all(Array.from(new Set(body.ids)).map(async (id) => {
    ...
    if (entry.crawler !== "lcsc") return failed(entry, "Key component is not an LCSC crawler");
    if (!entry.enabled) return failed(entry, "Key component crawler is disabled");
    return fetchLcscPrice(entry);
  }));
  ...
  if (isDevelopment) {
    for (const result of results) {
      ...
      if (result.success) console.log(`[LCSC API] result ${details}`);
      else console.error(`[LCSC API] result failed ${details}`);
    }
  }
